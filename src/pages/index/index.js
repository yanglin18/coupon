...
export default {
  ...
  methods: {
    ...
    // 优惠价格
    producePrice (item) {
      return (item.price * item.buy_number).toFixed(2);
    },
    // 原价格
    ...
    To_buy1 () {
      console.log("tobuy1", this.is_getuserInfo);
      // #ifdef MP-TOUTIAO
    ...
    GetPhoneNumber (res0) {
      if (res0.detail.iv)
      {
        uni.showLoading({
          ...
          mask: true,
          success: () => {
            console.log("showLoading生效");
          },
          fail: () => {
            console.log("showLoading失败");
          }
        });
