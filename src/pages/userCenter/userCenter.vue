<template>
  <view class="main" v-bind:style="{
      paddingTop: navHeight + 'px'
    }">
    <navigationbar class="navbar" :status_img="skin.status_img" :title="text"></navigationbar>
    <view class="content">
      <image :src="skin.img" :default-source="skin.img" class="allBgImg" v-bind:style="{
          paddingTop: navHeight + 'px',
      ...
      <view>
        <view class="top_card">
          ...
          <view class="bottom" @click="NavToModify">
            <view class="left">
              ...
              <!-- #ifndef MP-ALIPAY -->
              <button size="mini" open-type="getUserInfo" @getuserinfo="GetUserInfo" @click="ByteLoginIn"
                v-if="!hasLogin1" class="Login_in">
                登录
              </button>
              <!-- #endif -->
              <!-- #ifdef MP-ALIPAY -->
              <button size="mini" open-type="getAuthorize" @getAuthorize="GetAuthorize" @error="onAuthError"
                scope="userInfo" v-if="!hasLogin1" class="Login_in">
                登录
              </button>
              <!-- #endif -->
        ...
        <view class="about">
          ...
          <!-- #endif -->
          <!-- #ifdef MP-WEIXIN -->
          <button class="service contact deline" open-type="contact" bindcontact="contactMKX()">
            <view class="left">
              <image class="leftImage" src="../../static/assets/service.png" />
              <text>
                联系客服
      ...
      <view v-if="contactUS" class="contactUS">
        <view class="contactTop">
          <view class="Phone_number">
            <text @click="callUS">手机号：<text style="color: #42B069;">13570303413</text></text>
          </view>
    ...
    methods: {
      ...
      getBanner()
      {
        this.Ajax("post", "member/index/banner", {}, res =>
        {
          console.log(res);
          if (res.data.code === "200") {
            this.shareImg = res.data.data.share;
          }
